...
Graph-building utilities live under `src/graphs/` and currently focus on materializing directed k-nearest-neighbor graphs from any `AbstractANNIndex`.
...
- **Construction**: use `build_knn_graph(index, data; k=..., include_self=false, metadata=nothing)` to query every column of `data` through the supplied index.
- **Resolving `k`**:
...
- **Metadata**:
  ...
  - Use `has_metadata(graph)`, `graph_metadata(graph)`, or `node_metadata(graph, i)` to consume the payload safely.
