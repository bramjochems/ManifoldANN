...
#
# Usage: bash scripts/cloud/vm_progress.sh <run-id>
#        bash scripts/cloud/vm_progress.sh <run-id> --watch   (refresh every 60s)

set -euo pipefail

RUN_ID="${1:?usage: vm_progress.sh <run-id> [--watch]}"
WATCH=0
[ "${2:-}" = "--watch" ] && WATCH=1
